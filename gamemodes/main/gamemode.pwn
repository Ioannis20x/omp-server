...
public OnPlayerDisconnect(playerid, reason)
{
	...
	GetPlayerPos(playerid,x,y,z);
	for(new i = 0, j = MAX_PLAYERS; i <= j; i++) //GetPlayerPoolSize ist die h�chste online playerid   | i repr�sentiert die aktuelle id die gecheckt wird
    {
...
public OnPlayerSpawn(playerid)
{
    ...
    if (sInfo[playerid][eingeloggt] == 0)
    {
        ...
        if(sInfo[playerid][padminlevel]>=2){
			SendClientMessage(playerid,COLOR_RED,"SERVER:{FFFFFF} Du bist als Supporter eingeloggt. Vielen Dank f�r dein Engagement!");
		}
		...
		if(sInfo[playerid][pfleader]>0){
			if(sInfo[playerid][pfraktion]==7){
                SendClientMessage(playerid,COLOR_RED,"SERVER:{FFFFFF} Willkommen zur�ck Herr Pr�sident.");
			}else{
                SendClientMessage(playerid,COLOR_RED,"SERVER:{FFFFFF} Du bist als Fraktionsleitung eingeloggt. Vielen Dank f�r dein Engagement!");
			}
		}
		wstring = "{FFFFFF}Willkommen auf "#SERVERNAME "!\n Das Team w�nscht dir viel Spa� beim spielen!\n Bei Fragen oder Problemen kannst du uns gerne auf Discord anschreiben {FF0000}(/discord).";
		ShowPlayerDialog(playerid, 563, DIALOG_STYLE_MSGBOX, "Willkommen auf "#SERVERNAME , wstring,"Okay","");
		
		
		
		#if defined Weihnachten
			SendClientMessage(playerid, COLOR_RED, "SERVER: {FFFFFF}" #SERVERNAME " w�nscht dir frohe Weihnachten.");
		#endif
...
public OnPlayerDeath(playerid, killerid, WEAPON:reason){
 ...
 //   new Float:x, Float:y, Float:z;
	...
	SendClientMessage(playerid,COLOR_RED,string);
	SendClientMessage(killerid, COLOR_HRED,"Du hast ein Verbrechen begangen ( Vors�tzlicher Mord ). Reporter: Anonym.");
...
CMD:intinfo(playerid,params[])
{
	...
	format(istring,sizeof(istring),"Interior Schl�ssel: %i",iID);
...
CMD:showfill(playerid,params[])
{
	...
	format(string,sizeof(string),"Deine Tankf�llung entspricht %i%",tank[vID]);
...
CMD:suicide(playerid,params[])
{
	...
	SCM(playerid,COLOR_RED,"SERVER: Selbstmord ist keine L�sung! Oder etwa doch?!");
	return 1;
}
	...
	if(strcmp(option,"lock",false)==0)
{
    ...
    printf("Kein g�ltiges Fahrzeug gefunden!");
    return 1;
}
...
SendClientMessage(playerid,COLOR_HBLUE,"[______________Haus�bersicht______________]");
for(new i=0;i<=sizeof(hInfo);i++){
...
{
new string[256];
SendClientMessage(playerid,COLOR_HBLUE,"[______________Fahrzeug�bersicht______________]");
for(new i=0;i<=MAX_VEHICLES;i++){
...
SendClientMessage(playerid,COLOR_HRED,"Gegr�ndet von: "#SERVEROWNER ".");
...
CMD:clear(playerid,params[])
{
	...
	format(string,sizeof(string),"HQ: An alle Einheiten: Verd�chtiger %s wurde von %s freigestellt!",getPlayerName(pID),getPlayerName(playerid));
...
if(IsPlayerInRangeOfPoint(playerid,10.0,1038.0804,-1339.8496,13.7343)&& isPlayerinStaat(playerid))
{
    ...
    if(GetPlayerMoney(playerid)< preis)return SendClientMessage(playerid,COLOR_RED,"FEHLER: {FFFFFF}Du hast nicht genug Geld dabei.");
 	format(string,sizeof(string),"* Du hast %i Donut(s) f�r $%i gekauft. (Donuts im Inventar: %i)",anzahl,preis,sInfo[playerid][pdonut]);
	SendClientMessage(playerid,COLOR_YELLOW,string);
...
CMD:use(playerid,params[])
{
	...
	{
	if(sInfo[playerid][pgreen] < 2)return SendClientMessage(playerid, COLOR_RED,"FEHLER: {FFFFFF}Du ben�tigst mindestens 2g Green.");
    new Float:x, Float:y, Float:z;
    GetPlayerPos(playerid, x,y,z);
    GetPlayerHealth(playerid,health);
...
SendClientMessage(playerid,COLOR_YELLOW,"SPAWNCHANGE: {FFFFFF}W�hle deinen Spawn aus.");
return 1;
}
...
//	 motorhaube
	...
	if(tmp_bonnet==true){
		...
		tmp_bonnet = true;
		SendClientMessage(playerid,COLOR_GREY,"INFO: Motorhaube ge�ffnet.");
		}
	SetVehicleParamsEx(vID,tmp_engine, tmp_lights, tmp_alarm, tmp_doors, tmp_bonnet, tmp_boot, tmp_objective);
...
if(!strcmp(item,"kraum",false))
	...
	tmp_boot=true;
	SetVehicleParamsEx(vID, tmp_engine, tmp_lights,tmp_alarm,tmp_doors,tmp_bonnet,tmp_boot,tmp_objective);
   	SendClientMessage(playerid,COLOR_GREY,"Kofferraum ge�ffnet.");}
	else{
	if(tmp_boot == true){
	tmp_boot = false;
	SetVehicleParamsEx(vID, tmp_engine, tmp_lights,tmp_alarm,tmp_doors,tmp_bonnet,tmp_boot,tmp_objective);
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
	if(GetPVarInt(playerid,"trash_job"))
	{
		...
		DisablePlayerCheckpoint(playerid);
		SendClientMessage(playerid,COLOR_YELLOW,"M�LLMANN: {FFFFFF}Route beendet.");
	}
...
if(GetPVarInt(playerid, "trash_job"))
	{
	    //Wenn trash job ausgef�hrt wird
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(response)
 		{
		    ...
		    if(listitem==1){
 			SetPlayerColor(playerid, COLOR_RED);
            GetPlayerName(playerid, name, sizeof(name));
            format(string, sizeof(string), "SERVER: %s {FFFFFF}ist nun als Gro�mogul im Dienst.", name);
            if(!isaduty(playerid)){
			SendClientMessageToAll(COLOR_RED, string);
			SetPVarInt(playerid,"aduty",1);
			TextDrawShowForPlayer(playerid,aduty_text);
			...
			return 1;
 			...
 			SetPlayerColor(playerid, COLOR_RED);
            GetPlayerName(playerid, name, sizeof(name));
            format(string, sizeof(string), "SERVER: %s {FFFFFF}ist nun als gr��te Instanz des Servers im Dienst.", name);
            if(!isaduty(playerid)){
			SendClientMessageToAll(COLOR_RED, string);
			SetPVarInt(playerid,"aduty",1);
			TextDrawShowForPlayer(playerid,aduty_text);
	...
	if(dialogid==DIALOG_STREAM)
	{
		if (response)
		{
			...
			SendClientMessage(playerid,COLOR_HGREEN,"Du hast das Radio angeschaltet. Viel Spa� beim H�ren!");
...
{
SendClientMessage(playerid,0xFF9A00FF,"{35793B}GL�CKWUNSCH: Dieser Befehl macht garnichts!");
return 1;
}
/*
